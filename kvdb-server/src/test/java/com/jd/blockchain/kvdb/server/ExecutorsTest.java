...
public class ExecutorsTest {
    ...
    private Session newSession() {
        return context.getSession(UUID.randomUUID().toString(), key -> new DefaultSession(key, null, context.getDB(0)));
    }
    ...
    public void testSelect() {
        ...
        Response response = execute(session, new PutExecutor(), KVDBMessage.put(Bytes.fromString("k"), Bytes.fromString("v")));
        Assert.assertEquals(Constants.SUCCESS, response.getCode());
        response = execute(session, new ExistsExecutor(), KVDBMessage.exists(Bytes.fromString("k")));
        Assert.assertEquals(Constants.SUCCESS, response.getCode());
        Assert.assertEquals(1, response.getResult().length);

        response = execute(session, new SelectExecutor(), KVDBMessage.select(Bytes.fromInt(1)));
        Assert.assertEquals(Constants.SUCCESS, response.getCode());
        response = execute(session, new ExistsExecutor(), KVDBMessage.exists(Bytes.fromString("k")));
        Assert.assertEquals(Constants.SUCCESS, response.getCode());
        Assert.assertEquals(1, response.getResult().length);
