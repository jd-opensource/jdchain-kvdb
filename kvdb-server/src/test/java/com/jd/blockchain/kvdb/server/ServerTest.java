...
public class ServerTest {
	...
	public void testSingle() throws Exception {
		KVDBServerContext context = new KVDBServerContext(
				new ServerConfig(this.getClass().getResource("/server/single").getFile()));
		...
		NettyClient client = new NettyClient(new ClientConfig("localhost", config.getPort(), ""),
				() -> reqCdl.countDown());
		reqCdl.await();
		Response response = client.send(KVDBMessage.use("test1"));
		Assert.assertEquals(Constants.SUCCESS, response.getCode());
		DatabaseClusterInfo clusterInfo = BinaryProtocol.decodeAs(response.getResult()[0].toBytes(),
				DatabaseClusterInfo.class);
	...
	public void testCluster() throws Exception {
		KVDBServerContext context1 = new KVDBServerContext(
				new ServerConfig(this.getClass().getResource("/server/cluster/1").getFile()));
		// clean dirs;
		...
		KVDBServerContext context2 = new KVDBServerContext(
				new ServerConfig(this.getClass().getResource("/server/cluster/2").getFile()));
		// clean dirs;
